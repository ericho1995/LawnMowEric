...
// A suburb dropped from src/data/suburbs.mjs must not linger online: delete
// generated suburb pages this build no longer produces. Only files carrying
// the build's own GENERATED header are ever removed.
const produced = new Set(pages.map((p) => p.meta.path));
const removed = [];
if (existsSync(at('lawn-mowing'))) {
  for (const f of readdirSync(at('lawn-mowing'))) {
    const rel = 'lawn-mowing/' + f;
    if (!f.endsWith('.html') || produced.has(rel)) continue;
    if (readFileSync(at(rel), 'utf8').slice(0, 400).includes('GENERATED by scripts/build.mjs')) {
      unlinkSync(at(rel));
      removed.push(rel);
    }
  }
